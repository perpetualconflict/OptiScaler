...
$source = Join-Path $PSScriptRoot 'dlssd_publication_plan_probe.cpp'
$object = Join-Path $OutputDirectory 'dlssd_publication_plan_probe.obj'
$output = Join-Path $OutputDirectory 'dlssd_publication_plan_probe.exe'
...
if ($LASTEXITCODE -ne 0) { throw 'Publication plan probe build failed.' }
